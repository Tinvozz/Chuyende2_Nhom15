...
function create_distance_matrix_return(ins::instance, nodelist::Vector{T},pi_o::Vector{Float64},sigma_o::Vector{Float64}) where {T<:AbstractNode}
    ...
    for n1 in nodelist, n2 in nodelist
        ...
        if (n2.i == 1) && (n1.loc != n2.loc)
            distance_matrix[n1.id + 1, n2.id + 1] = dist_locs_by_bottom(n1.loc,n2.loc,ins) - pi_o[n2.loc.id] - sigma_o[n2.loc.id]
        elseif (n1.loc == n2.loc) && (n2.i == n1.i + 1)
            distance_matrix[n1.id + 1, n2.id + 1] = dist_locs_by_bottom(n1.loc,n2.loc,ins) - pi_o[n2.loc.id]
        end
    end
    # with iopoint
    for n in nodelist
...
function feasible_extension(path::Partial_path_return, new_node::Node)
...
function update_loc_reachability(path::Partial_path_return,old_node::Node)
...
function extend(path::Partial_path_return,new_node::Node)
